...
// pressing ESCAPE.
//   Because hovering detection is disabled outside the popup, when clicking
//   outside the click will not be seen by underlying widgets! (*1)
// - Their visibility state (~bool) is held internally by Dear ImGui instead of
// being held by the programmer as we are used to with regular Begin() calls.
//   User can manipulate the visibility state by calling OpenPopup(),
//   CloseCurrentPopup() etc.
// - We default to use the right mouse (ImGuiMouseButton_Right=1) for the Popup
// Context functions. Those three properties are connected: we need to retain
// popup visibility state in the library because popups may be closed as any
// time.
// (*1) You can bypass that restriction and detect hovering even when normally
// blocked by a popup.
//      To do this use the ImGuiHoveredFlags_AllowWhenBlockedByPopup when
//      calling IsItemHovered() or IsWindowHovered(). This is what
//      BeginPopupContextItem() and BeginPopupContextWindow() are doing already,
//      allowing a right-click to reopen another popups without losing the
//      click.
...
IMGUI_API bool BeginPopupContextVoid(
    const char*      str_id = NULL,
    ImGuiMouseButton mouse_button =
        1); // helper to open and begin popup when clicked in void (where there
...
struct ImDrawCmd
{
    ...
    unsigned int
        VtxOffset; // Start offset in vertex buffer. Pre-1.71 or without
                   ...
                   // support meshes larger than 64K vertices with 16-bit
                   // indices.
...
struct ImFontAtlas
{
    ...
    // docs/FONTS.txt for more details about using colorful icons. Note: this
    // API may be redesigned later in order to support multi-monitor varying DPI
    // settings.
